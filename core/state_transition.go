...
func (st *StateTransition) distributeTxFees() error {
	...
	if feeCurrency == nil {
		...
		if rules.IsCel2 {
			st.state.AddBalance(feeHandlerAddress, baseTxFeeU256)
		} else if st.evm.ChainConfig().Optimism != nil {
			st.state.AddBalance(params.OptimismBaseFeeRecipient, baseTxFeeU256)
		}
		...
		if l1Cost != nil {
			st.state.AddBalance(params.OptimismL1FeeRecipient, l1CostU256)
		}
	} else {
		if l1Cost != nil {
