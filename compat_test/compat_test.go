...
func TestCompatibilityOfChains(t *testing.T) {
	...
	g.Go(func() error {
		for i := startBlock; i <= endBlock; i++ {
			index := i
			g.Go(func() error {
				err := fetchBlockElementsWithRetry(clients, index, resultChan)
				if err != nil {
...
func fetchBlockElementsWithRetry(clients *clients, blockNumber uint64, resultChan chan *blockResults) error {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	return retryOperationError(ctx, func() error {
		return fetchBlockElements(clients, blockNumber, resultChan)
	}, 5, time.Minute*2)
}
