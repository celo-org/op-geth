...
func (api *FilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	if crit.BlockHash != nil {
		...
		if header == nil || api.sys.backend.ChainConfig().IsOptimismPreBedrock(header.Number) {
		...
		if api.sys.backend.ChainConfig().IsOptimismPreBedrock(big.NewInt(begin)) {
			if api.sys.backend.ChainConfig().IsOptimismPreBedrock(big.NewInt(end)) {
				return api.getLogsFromHistoricalService(ctx, crit)
			}
			// Need to merge
			crit.ToBlock.Sub(api.sys.backend.ChainConfig().BedrockBlock, common.Big1)
			preMigrationLogs, err := api.getLogsFromHistoricalService(ctx, crit)
			if err != nil {
				return nil, err
			}
			begin = api.sys.backend.ChainConfig().BedrockBlock.Int64()
			filter = api.sys.NewRangeFilter(begin, end, crit.Addresses, crit.Topics)
