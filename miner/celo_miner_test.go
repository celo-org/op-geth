...
func createCeloMiner(t *testing.T) *Miner {
	...
	statedb, _ := bc.StateAt(bc.Genesis().Root())
...
func TestMinerFeeCalculationWithCurrencyConversion(t *testing.T) {
	...
	r := miner.generateWork(context.Background(), &generateParams{
...
func TestBlocklistOnlyForSequencing(t *testing.T) {
	...
	r2 := miner.generateWork(context.Background(), &generateParams{
		parentHash: parentBlock.Hash(),
		timestamp:  parentBlock.Time + 1,
		random:     common.HexToHash("0xcafebabe"),
		noTxs:      true,
	...
	r3 := miner.generateWork(context.Background(), &generateParams{
		parentHash: parentBlock.Hash(),
		timestamp:  parentBlock.Time + 1,
		random:     common.HexToHash("0xcafebabe"),
		noTxs:      false,
